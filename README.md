...
**Phase 0 — 툴킷 구축 + 요구사항 수집 준비.** `bench`로 실제 Frappe 인스턴스나
`apps/babipa_erp/` 커스텀 앱은 아직 만들지 않았다. 지금까지 확정된 것:
...
다음 단계는 요구사항수집 xlsx를 유관 부서에 배포해 실 데이터를 걷는 것 →
`bench new-app babipa_erp`로 실제 스캐폴딩 시작.
...
└── (향후) apps/babipa_erp/          # bench new-app 실행 시 생성될 실제 앱 코드 위치
```
