...
2. Claude Code 개발 툴킷 완성 — 서브에이전트 10개 + 슬래시커맨드 9개 + 스킬 8개 (`.claude/` 아래, 아래 §1 참조)
...
│   ├── 품목코드_260720.xlsx          # 실 품목 마스터(289건) — .gitignore 등록, 저장소엔 없음(로컬 전용)
│   ├── item-doctype-design.md       # 위 xlsx 기준 Item DocType 필드 매핑/Item Group 설계 (2026.07.20)
│   └── research/                   # AI 딥리서치 보고서(참고자료, 미확정 문서 포함)
│       ├── ERPNext_및_Claude_Code_가이드.md            # 채택안의 근거자료
│       ├── Python_ERP_시스템_설계(대안,미채택).md        # 검토 후 반려된 대안
│       ├── ERP_지식정리_자동차전장부품_ERPNext구축관점(v15기준).md   # 업무 도메인 배경지식 (v15 기준)
│       └── ERPNext_가이드_검토_및_v16_빌드권고_2026-07-16.md         # 위 문서의 v16 검토·보정
...
└── skills/                    # 8개 스킬 (모델이 자동으로 판단해 로드)
    ...
    └── frappe-v16-breaking-changes/SKILL.md          # v16 갭(Serial/Batch Bundle 등, 2026.07.20 신설)
```
...
이 프로젝트 착수 전후로 수행한 AI 딥리서치 보고서 4건. **운영 지침이 아니라 참고자료**이며,
실제 결정은 항상 `CLAUDE.md`와 `docs/decisions.md`가 우선한다.
...
| `ERP_지식정리_자동차전장부품_ERPNext구축관점(v15기준).md` | 배경지식, 일부 수정 반영됨 | 회계·제조·재고·구매·판매·HR·품질·기술아키텍처·도입방법론 9개 카테고리 정리. v15 기준이라 아래 문서로 보정 필요 |
| `ERPNext_가이드_검토_및_v16_빌드권고_2026-07-16.md` | 검토 완료, 반영 사항 다수 | 위 문서의 v16 사실 검증·보정 + Claude Code 빌드 가이드. 2026.07.20 반영분(라이선스 정정, v16 breaking changes 스킬)과 미결정 항목은 `docs/decisions.md` 참조 |
